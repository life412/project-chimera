...
# 🧠 PLANNING BEFORE EXECUTION (BORIS CHERNY WORKFLOW)

For any non-trivial task (multi-step, architectural, or involving changes across files):

1. The agent MUST first produce a clear execution plan.
2. The plan should include:
   - Objective of the task
   - Steps to complete it
   - Risks or assumptions
   - Verification strategy (tests, checks, validation steps)
3. The agent MUST wait for user approval before executing the plan unless explicitly told to proceed.

This rule exists to reduce misalignment and rework.

---

# ✅ VERIFICATION & OUTPUT ACCOUNTABILITY

Before finalizing any response involving code, configuration, or system behavior:

- The agent MUST propose at least one verification method.
Examples include:
- Unit tests
- Manual test steps
...
No solution is considered complete without a verification path.

---

# 🤖 AGENT ROLE SPECIALIZATION

When appropriate, the agent may assume specialized roles, such as:

- Planner: break down complex problems
- Implementer: write code based on an approved plan
- Reviewer: analyze code for correctness and risks
- Documenter: generate README or markdown explanations

Roles may be executed sequentially within a single response when helpful.


---

# 🔁 RULE EVOLUTION & CONTINUOUS IMPROVEMENT

This rules file is a living document.

- If the agent produces incorrect, ambiguous, or misaligned output, the failure MUST be documented.
- A corrective rule should be added to this file to prevent repetition.
- Rules may be refined, merged, or removed as patterns evolve.
- Changes should be committed to version control with clear messages.

The goal is continuous alignment between human intent and agent behavior.


---

# 🧾 COMMUNICATION & CODING CONVENTIONS

- Prefer clarity over brevity.
- Avoid assumptions; ask clarifying questions when intent is unclear.
- Use descriptive variable and function names.
- Explain reasoning for non-obvious decisions.
- When uncertain, surface trade-offs instead of guessing.
